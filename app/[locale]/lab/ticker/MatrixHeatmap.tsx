"use client";

import { useEffect, useRef, useState } from "react";

export interface MockOffer {
  vendor: string;
  codice: string;
  price: number;
  median: number;
  category: string;
  isSpread: boolean;
}

export interface Section {
  title: string;
  unit: string;
  offers: MockOffer[];
}

const NUMBER_4DP = new Intl.NumberFormat("it-IT", {
  minimumFractionDigits: 4,
  maximumFractionDigits: 4,
});

function colorForDelta(price: number, median: number): { fill: string; glow: string } {
  const delta = (price - median) / median;
  if (delta < -0.3)
    return { fill: "#14d97a", glow: "rgba(20, 217, 122, 0.8)" }; // bright green
  if (delta < -0.1)
    return { fill: "#10b981", glow: "rgba(16, 185, 129, 0.5)" }; // green
  if (delta < 0.1) return { fill: "#facc15", glow: "rgba(250, 204, 21, 0.5)" }; // amber
  if (delta < 0.3) return { fill: "#fb923c", glow: "rgba(251, 146, 60, 0.6)" }; // orange
  return { fill: "#f43f5e", glow: "rgba(244, 63, 94, 0.8)" }; // red
}

export function MatrixHeatmap({ sections }: { sections: Section[] }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [hovered, setHovered] = useState<MockOffer | null>(null);

  // Matrix rain background
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    const resize = () => {
      canvas.width = window.innerWidth;
      canvas.height = window.innerHeight;
    };
    resize();
    window.addEventListener("resize", resize);

    const fontSize = 14;
    const columns = Math.floor(canvas.width / fontSize);
    const drops: number[] = Array(columns).fill(1).map(() => Math.random() * -50);
    const chars =
      "01ABCDEFGHIJKLMNOPQRSTUVWXYZ€$≈+-↑↓0123456789kWhSmc".split("");

    let animationId = 0;
    const draw = () => {
      ctx.fillStyle = "rgba(0, 0, 0, 0.08)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      ctx.font = `${fontSize}px JetBrains Mono, ui-monospace, monospace`;

      for (let i = 0; i < drops.length; i++) {
        const char = chars[Math.floor(Math.random() * chars.length)];
        // Top character glowing white-ish, trail in green
        ctx.fillStyle =
          drops[i] < 5
            ? "rgba(255, 255, 255, 0.6)"
            : "rgba(20, 217, 122, 0.35)";
        ctx.fillText(char, i * fontSize, drops[i] * fontSize);
        if (drops[i] * fontSize > canvas.height && Math.random() > 0.975) {
          drops[i] = 0;
        }
        drops[i]++;
      }
      animationId = requestAnimationFrame(draw);
    };
    draw();

    return () => {
      window.removeEventListener("resize", resize);
      cancelAnimationFrame(animationId);
    };
  }, []);

  let globalIndex = 0;
  const totalOffers = sections.reduce((s, sec) => s + sec.offers.length, 0);

  return (
    <div className="relative min-h-screen bg-black overflow-x-hidden">
      <canvas
        ref={canvasRef}
        className="fixed inset-0 pointer-events-none opacity-50"
        aria-hidden="true"
      />

      <div className="relative z-10 p-4 sm:p-8 max-w-[1600px] mx-auto">
        <header className="mb-8 border-b border-emerald-400/30 pb-4">
          <h1 className="text-3xl sm:text-5xl font-bold text-emerald-400 font-mono tracking-wider">
            MARKET MAP
          </h1>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-emerald-300/70 font-mono text-xs sm:text-sm">
            <span>{totalOffers} OFFERS</span>
            <span>·</span>
            <span>PLACET DOMESTICO</span>
            <span>·</span>
            <span>SORT: PRICE ASCENDING</span>
            <span>·</span>
            <span>LAB/PROTOTYPE</span>
          </div>
          <div className="flex flex-wrap items-center gap-4 mt-3 text-xs font-mono">
            <span className="flex items-center gap-1.5">
              <span
                className="inline-block w-3 h-3 rounded-sm"
                style={{ background: "#14d97a", boxShadow: "0 0 6px rgba(20,217,122,0.7)" }}
              />
              <span className="text-emerald-300/80">{"-30%+ vs mediana"}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="inline-block w-3 h-3 rounded-sm"
                style={{ background: "#10b981" }}
              />
              <span className="text-emerald-300/80">{"-30% .. -10%"}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="inline-block w-3 h-3 rounded-sm"
                style={{ background: "#facc15" }}
              />
              <span className="text-emerald-300/80">{"mediana ±10%"}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="inline-block w-3 h-3 rounded-sm"
                style={{ background: "#fb923c" }}
              />
              <span className="text-emerald-300/80">{"+10% .. +30%"}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span
                className="inline-block w-3 h-3 rounded-sm"
                style={{ background: "#f43f5e", boxShadow: "0 0 6px rgba(244,63,94,0.7)" }}
              />
              <span className="text-emerald-300/80">{"+30% e oltre"}</span>
            </span>
          </div>
        </header>

        <div className="space-y-10">
          {sections.map((section) => {
            const best = section.offers[0];
            const worst = section.offers[section.offers.length - 1];
            const medianValue = section.offers[Math.floor(section.offers.length / 2)].price;
            return (
              <section key={section.title}>
                <div className="flex items-baseline justify-between mb-3 border-b border-emerald-400/15 pb-2">
                  <h2 className="text-lg sm:text-xl font-mono font-bold text-emerald-300 tracking-wider">
                    {section.title}
                  </h2>
                  <span className="font-mono text-xs sm:text-sm text-emerald-300/60 tabular-nums">
                    {section.offers.length} OFFERTE · MED {section.offers[0].isSpread ? "+" : ""}
                    {NUMBER_4DP.format(medianValue)} {section.unit} · BEST{" "}
                    {section.offers[0].isSpread ? "+" : ""}
                    {NUMBER_4DP.format(best.price)} · WORST{" "}
                    {section.offers[0].isSpread ? "+" : ""}
                    {NUMBER_4DP.format(worst.price)}
                  </span>
                </div>

                <div
                  className="grid gap-[3px]"
                  style={{
                    gridTemplateColumns: "repeat(auto-fill, minmax(22px, 1fr))",
                  }}
                >
                  {section.offers.map((o) => {
                    const { fill, glow } = colorForDelta(o.price, o.median);
                    const idx = globalIndex++;
                    return (
                      <button
                        key={o.codice}
                        type="button"
                        className="aspect-square rounded-[3px] cursor-pointer relative tile-fall focus:outline-none focus:ring-2 focus:ring-emerald-400"
                        style={{
                          background: fill,
                          boxShadow: `0 0 6px ${glow}`,
                          animationDelay: `${idx * 3}ms`,
                        }}
                        onMouseEnter={() => setHovered(o)}
                        onMouseLeave={() => setHovered(null)}
                        onFocus={() => setHovered(o)}
                        onBlur={() => setHovered(null)}
                        aria-label={`${o.vendor} ${NUMBER_4DP.format(o.price)} ${section.unit}`}
                      />
                    );
                  })}
                </div>
              </section>
            );
          })}
        </div>

        <footer className="mt-12 text-center text-emerald-300/40 font-mono text-xs">
          ENERGY INDEX · MARKET MAP LAB · DATI MOCK · {new Date().toISOString().slice(0, 10)}
        </footer>
      </div>

      {hovered && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-black/95 border-2 border-emerald-400/60 text-emerald-300 font-mono px-6 py-3 rounded-lg backdrop-blur-md z-30 shadow-[0_0_30px_rgba(20,217,122,0.4)] pointer-events-none min-w-[280px]">
          <div className="text-base sm:text-lg font-bold text-emerald-400 tracking-wider">
            {hovered.vendor}
          </div>
          <div className="text-xs text-emerald-300/60 mb-2">
            {hovered.codice} · {hovered.category}
          </div>
          <div className="text-2xl sm:text-3xl font-bold tabular-nums">
            {hovered.isSpread ? "+" : ""}
            {NUMBER_4DP.format(hovered.price)}{" "}
            <span className="text-sm font-normal text-emerald-300/70">
              €
            </span>
          </div>
          <div className="text-xs text-emerald-300/70 tabular-nums mt-1">
            Δ vs mediana:{" "}
            <span
              style={{
                color:
                  hovered.price < hovered.median
                    ? "#14d97a"
                    : hovered.price > hovered.median * 1.1
                      ? "#f43f5e"
                      : "#facc15",
              }}
            >
              {((hovered.price - hovered.median) / hovered.median * 100).toFixed(1)}
              %
            </span>
          </div>
        </div>
      )}

      <style
        // eslint-disable-next-line react/no-unknown-property
        dangerouslySetInnerHTML={{
          __html: `
            @keyframes tilefall {
              from { opacity: 0; transform: translateY(-60px) scale(0.6); }
              60%  { opacity: 1; transform: translateY(4px) scale(1.02); }
              to   { opacity: 1; transform: translateY(0) scale(1); }
            }
            .tile-fall {
              opacity: 0;
              animation: tilefall 0.6s cubic-bezier(0.3, 0.6, 0.3, 1) forwards;
            }
            .tile-fall:hover {
              animation: none;
              opacity: 1;
              transform: scale(2.5);
              z-index: 20;
            }
          `,
        }}
      />
    </div>
  );
}
